...
class ImageEncoder(object):
    def __init__(self, checkpoint_filename, input_name="images", output_name="features"):
        ...
        config.gpu_options.allow_growth = True  # Chỉ cấp phát bộ nhớ GPU khi cần
        self.session = tf.compat.v1.Session(config=config)
        with tf.gfile.GFile(checkpoint_filename, "rb") as file_handle:
            graph_def = tf.GraphDef()
            graph_def.ParseFromString(file_handle.read())
        tf.import_graph_def(graph_def, name="net")
        self.input_var = tf.get_default_graph().get_tensor_by_name( input_name + ':0')
        self.output_var = tf.get_default_graph().get_tensor_by_name(output_name + ':0')
